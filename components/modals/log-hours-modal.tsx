...
export function LogHoursModal({ isOpen, equipmentId, onClose }: LogHoursModalProps) {
  const insets = useSafeAreaInsets();
  const { appState, updateEquipmentHours } = useAppData();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateEquipmentHours(selectedEquipmentId, newHours);

      Alert.alert("Uspjeh", "Sati su uspješno zabilježeni", [
        {
          text: "OK",
          onPress: () => {
            setHours("");
      ...
      Alert.alert("Greška", "Greška pri spremanju sati");
    } finally {
      setLoading(false);
    }
  };
